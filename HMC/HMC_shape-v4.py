...
#       N  - HMC chain length                                         #
#                                                                     #
# Visibilities are simulated using Montblanc (version 4 - single GPU) #
...
#scalelength prior 
#mean_v = 0.266*ARCS2RAD 
std_v = 0.3136*ARCS2RAD #0.496*ARCS2RAD 
...
parser = argparse.ArgumentParser(description='MCMC-LENSING')
...
parser.add_argument('-N',dest='N', type=int, default=100, help='MCMC chain length')
...
# Brightness matrix for sources in muJy
    ...
    # Create observed_visibilities and upload it to the GPU
    slvr.solve()
    with slvr.context:
    ...
    # Posterior
    def logPostFunc(q):
        sersic_shape = q.reshape((3,nssrc))
        slvr.transfer_sersic_shape(sersic_shape)
        #slvr.point_errors[:] = point_errors
        slvr.solve() 
        loglike = -0.5*slvr.X2
